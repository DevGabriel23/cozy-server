...
export function generateAddonImageFallback(item: any, isNew: boolean = false): string {
    const textToRender = isNew ? `✨ NUEVO\n${item.data.title}` : item.data.title;
    // Si no, generamos el fallback
    const uriEncodedTitle = encodeURIComponent(textToRender);
